/*  ************************************************************************************
    ...
    FILE INFO
        A process that creates threads and uses semaphores to lock them such that no 
		two processes are in their critical section at the same time.
    ************************************************************************************
*/
#include <unistd.h>
#include <stdio.h>
...
int main(int argc, char *argv[]) {
	...
	while (threadsLeft(threads, thread_Count) > 0)
	{
		...
		while ((i = threadToStart(threads, thread_Count)) > -1) {
			...
			if (f_thread) {
				//then its no longer the first thread
                f_thread = false;
				//post up wit my even and odd homies
...
//helper function given | print when thread finishes
